...
| **Rewriter** | Named presets + optional `--attacker-instruction` | Fixed decomposition system prompt (or `--adversary-system-prompt-file`) |
...
| Flag | Purpose |
| --- | --- |
| `--results-dir` | Required unique output directory. |
| `--mode` | `train` \| `eval`. |
...
uv run python runs/adversary_run.py --mode eval --results-dir results/theme2/adv_eval

# Train, prompt-only (no LoRA)
uv run python runs/adversary_run.py --mode train --no-finetune \
  --adversary-prompt persona --results-dir results/theme2/adv_prompt_only

# Train + fine-tune (defaults from YAML)
uv run python runs/adversary_run.py --mode train --results-dir results/theme2/adv_ft

# Policy switches (examples)
uv run python runs/adversary_run.py --mode train --adversary-policy rloo --results-dir results/theme2/adv_rloo
uv run python runs/adversary_run.py --mode train --adversary-policy reinforce \
  --rs-min-successes 1 --rs-budget 5 --results-dir results/theme2/adv_rs

# Preset sweep (--no-finetune)
for PROMPT in default persona decompose academic fictional; do
  uv run python runs/adversary_run.py --mode train --no-finetune \
    --adversary-prompt "$PROMPT" --results-dir "results/theme2/adv_matrix_${PROMPT}"
done
```

**Throughput:** `--target_max_workers` affects how many target requests overlap **across examples**; raising it past GPU saturation can hurt latency (see [§8](#8-throughput-and-target-concurrency)).
...
| `--adversary-system-prompt-file` | Replace built-in decomposition system prompt. |
...
uv run python runs/adversary_v2.py --results-dir results/theme2/v2_eval

# Train + fine-tune
uv run python runs/adversary_v2.py --mode train --results-dir results/theme2/v2_ft

# Train, prompt-only / no LoRA
uv run python runs/adversary_v2.py --mode train --no-finetune --results-dir results/theme2/v2_prompt_only

# Policy switches (same pattern as v1)
uv run python runs/adversary_v2.py --mode train --adversary-policy rloo --results-dir results/theme2/v2_rloo
uv run python runs/adversary_v2.py --mode train --adversary-policy reinforce \
  --rs-min-successes 1 --rs-budget 5 --results-dir results/theme2/v2_rs

# Eval-method + decomposition knobs (examples)
uv run python runs/adversary_v2.py --eval-method heuristic --results-dir results/theme2/v2_heuristic
uv run python runs/adversary_v2.py --max-subquestions 8 --target-sub-workers 4 --results-dir results/theme2/v2_ablate
```
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| B1 | | REINFORCE (default batch) | WINNER1 | `--adversary-policy reinforce` | `results/theme2/B1_reinforce_w1` | | | | | |
| B2 | | REINFORCE | WINNER2 | | `results/theme2/B2_reinforce_w2` | | | | | |
| B3 | | REINFORCE, batch 4 | WINNER1 | `--adversary-reinforce-batch-size 4` | `results/theme2/B3_reinforce_b4_w1` | | | | | |
| B4 | | RLOO | WINNER1 | `--adversary-policy rloo` | `results/theme2/B4_rloo_w1` | | | | | |
| B5 | | RLOO | WINNER2 | | `results/theme2/B5_rloo_w2` | | | | | |
| B6 | | Rejection sampling | WINNER1 | `--rs-min-successes 1 --rs-budget 5` | `results/theme2/B6_rs_w1` | | | | | |

### Phase C — Hyper sweeps (optional)

| ID | Status | What varies | `--results-dir` (example) | Test ASR (final) | Notes |
| --- | --- | --- | --- | --- | --- |
| C1 | | `iterations` | `results/theme2/C1_iters_*` | | |
| C2 | | `lr` / `weight_decay` | `results/theme2/C2_lr_*` | | |
| C3 | | `rs_budget` / `rs_min_successes` | `results/theme2/C3_rs_*` | | |
| C4 | | `adversary_reinforce_batch_size` | `results/theme2/C4_batch_*` | | |
| C5 | | Custom `--attacker-instruction` | `results/theme2/C5_custom_instr` | | |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| D1 | | Default + judge | `uv run python runs/adversary_v2.py` | `results/theme2/D1_adv_v2_judge` | | | | judge | |
| D2 | | Heuristic eval | `--eval-method heuristic` | `results/theme2/D2_adv_v2_heuristic` | | | | heuristic | |
| D3 | | Ablation | `--max-subquestions` / `--target-sub-workers` | `results/theme2/D3_adv_v2_ablate` | | | | | |

### Phase E — Optional controls

| ID | Status | Experiment | Command | `--results-dir` (example) | Test ASR | Notes |
| --- | --- | --- | --- | --- | --- | --- |
| E1 | | CoEV v2 | `uv run python runs/coev_v2_run.py --mode coev` | `results/theme2/E1_coev_v2` | | |
| E2 | | `adversary_run` eval-only | `--mode eval` | `results/theme2/E2_eval_only` | | |
| E3 | | GEPA (if used) | | `results/theme2/E3_gepa_*` | | |
...
**Judge:** Decomposition-specific `[INST]` template (`ADVERSARY_V2_JUDGE_CLASSIFICATION_TEMPLATE`); not identical to classic single-reply HarmBench classifier wording.
