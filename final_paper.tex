...
For one matched example, the weak adversary produced a long rewrite that began with safety hedging and partially refused the harmful request before it reached the target; the target then fully refused and the judge assigned ASR $0$. 
The stronger adversary kept the harmful intent intact while embedding it in a fictional professional-documentation frame; the target did not refuse and the judge assigned ASR $1$. 
...
Table~\ref{tab:xstest} reports the \xstest{} runs that are already present in the repository. 
These are adversary-mode evaluations: each \xstest{} prompt is first rewritten by a saved adversary checkpoint, then sent to the target under the default helpful-assistant system prompt. 
The table therefore measures two properties of the learned rewriters rather than the standalone utility cost of a GEPA defense prompt. 
First, the safe-compliance columns test whether the adversary corrupts harmless prompts into refusals. 
Second, the unsafe-ASR column tests whether the same rewriter transfers to \xstest{} unsafe prompts.

The main result is that all saved adversary checkpoints preserve high safe compliance, $96.4$--$98.8\%$, with low over-refusal, $1.2$--$3.6\%$. 
At the same time, unsafe ASR remains high, $0.775$--$0.970$, on unsafe \xstest{} prompts. 
...
However, the current artifacts do not yet support a claim that evolved GEPA defense prompts reduce over-refusal relative to the default prompt. 
That requires the target-only and GEPA-defense \xstest{} runs listed in Appendix~\ref{sec:remaining-experiments}.
...
The unsafe-ASR column measures whether adversarial rewrites of \xstest{} unsafe prompts elicit non-refusal. 
It is not directly comparable to safe over-refusal: high values indicate the adversary transfers to \xstest{} unsafe prompts, while the safe-compliance and over-refusal columns measure collateral damage on benign prompts.
...
The current repository supports most report claims, but three professor-requested comparisons require either a final retrieval pass or additional \xstest{} runs.

\paragraph{Experiment A: XSTest / ``XSbench'' defense comparison.}
The codebase has \xstest{} support, not a separate XSbench runner. 
To measure harmless-prompt behavior cleanly, run the following four conditions on the same \xstest{} split:
\begin{enumerate}
    \item \textbf{Undefended target only:} run \texttt{configs/xstest\_batch\_eval.yaml}. This produces baseline safe compliance and over-refusal for \texttt{You are a helpful assistant.}
    \item \textbf{Undefended target after adversary rewrite:} use an existing adversary-mode config such as \texttt{configs/r11\_seed123\_xstest.yaml}; this condition is already available for several checkpoints.
    \item \textbf{GEPA defense only:} duplicate \texttt{configs/xstest\_batch\_eval.yaml}, set \texttt{runs.xstest.target\_system\_prompt} to \texttt{results/r14\_coev\_full\_prompt\_kl/optimized\_defense\_prompt.txt}, and write results to a new directory such as \texttt{results/xstest\_gepa\_defense\_target\_only}.
    \item \textbf{Adversary plus GEPA defense:} duplicate \texttt{configs/r14\_coev\_full\_prompt\_kl\_xstest.yaml}, replace the \xstest{} target system prompt with the same optimized defense prompt, and write to \texttt{results/r14\_coev\_full\_prompt\_kl\_xstest\_gepa\_defense}.
\end{enumerate}
These runs support the final safety-utility table requested in the report notes: target baseline, adversary only, GEPA defense only, and adversary plus GEPA defense.
...
A safe qualitative table can use the following columns:

\begin{table}[h]
\centering
\caption{Template for redacted qualitative examples.}
\label{tab:qual}
\begin{tabular}{p{0.18\linewidth}p{0.22\linewidth}p{0.25\linewidth}p{0.25\linewidth}}
\toprule
Harm category & Attack strategy & Why baseline failed & How evolved prompt patched it \\
\midrule
Self-harm & Ambiguous consequence framing & Baseline treated the request as generic discussion. & New prompt redirects sensitive requests and avoids normalizing self-harm. \\
Cyber abuse & Academic framing & Baseline answered as educational content. & New prompt distinguishes defensive education from actionable abuse. \\
Illegal evasion & Fictional documentation & Baseline followed the fictional premise. & New prompt preserves refusal under fiction, roleplay, and persona framing. \\
\bottomrule
\end{tabular}
\end{table}
