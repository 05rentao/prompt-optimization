...
The current repository supports most report claims, but some professor-requested comparisons require either a final retrieval pass or additional \xstest{} runs.

\paragraph{Experiment A: XSTest / ``XSbench'' defense comparison.}
The codebase has \xstest{} support, not a separate XSbench runner. 
The target-only baseline and GEPA-defense conditions have now been run on the same \xstest{} split and are reported in Table~\ref{tab:xstest}. 
The full four-condition matrix is:
\begin{enumerate}
    \item \textbf{Undefended target only:} run \texttt{configs/xstest\_batch\_eval.yaml}. This produces baseline safe compliance and over-refusal for \texttt{You are a helpful assistant.} This condition is complete.
    ...
    \item \textbf{GEPA defense only:} duplicate \texttt{configs/xstest\_batch\_eval.yaml}, set \texttt{runs.xstest.target\_system\_prompt} to the text in \texttt{results/r14\_coev\_full\_prompt\_kl/optimized\_defense\_prompt.txt}, and write results to a new directory such as \texttt{results/xstest\_gepa\_defense\_target\_only}. This condition is complete.
    \item \textbf{Adversary plus GEPA defense:} duplicate \texttt{configs/r14\_coev\_full\_prompt\_kl\_xstest.yaml}, replace the \xstest{} target system prompt with the same optimized defense prompt, and write to \texttt{results/r14\_coev\_full\_prompt\_kl\_xstest\_gepa\_defense}. This condition still requires the R14 co-evolution LoRA checkpoint.
\end{enumerate}
...
The completed target-only rows show that the evolved defense reduces direct unsafe compliance, but at a noticeable over-refusal cost.
